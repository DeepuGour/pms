...
class PropertiesTable extends Table
{
    ...
    public function validationDefault(Validator $validator): Validator
    {
        ...
        $validator
            ...
            ->notEmptyString('category_name','Please fill Category Name');

        $validator
            ->scalar('property_title')
            ->maxLength('property_title', 100)
            ->requirePresence('property_title', 'create')
            ->notEmptyString('property_title','Please fill Property Title');

        $validator
            ->scalar('property_description')
            ->maxLength('property_description', 255)
            ->requirePresence('property_description', 'create')
            ->notEmptyString('property_description','Please fill Property Description');

        $validator
            ->scalar('property_category')
            ->maxLength('property_category', 255)
            ->requirePresence('property_category', 'create')
            ->notEmptyString('property_category','Please fill Property Category');

        $validator
            ->scalar('property_tags')
            ->maxLength('property_tags', 255)
            ->requirePresence('property_tags', 'create')
            ->notEmptyString('property_tags','Please fill Property Tags');

        $validator
            ->requirePresence('image_file', 'create')
            ->notEmptyString('image_file','Please select image file')
            ->add('image_file',[
                'mimeType'=>[
                'rule'=>[
                    'mimeType',['image/jpg','image/png','image/jpeg'],
                    'message'=>'Please select image formate'],
                    'fileSize'=>[
                        'rule',['fileSize','<=','1mb'],
                        'message'=>'Please select image 1mb less',],
            ]]);
        $validator
            ->integer('status')
